...
Windows 的排程一天會跑兩次：15:00 主跑、17:30 重試。資料源偶爾在 15:00
還沒發布當天收盤資料（2026-09-03 實際發生過），主跑空轉時傍晚那次會補回來；
主跑成功的日子，重試會發現當天已處理過而直接跳過，提醒信也不會寄第二封。
就算兩次都落空（例如電腦整天沒開），隔天執行時也會用 `--catch-up`
把漏掉的交易日逐日補跑，不需要手動介入。
