...
## 2026-09-07：最終目標是現金流，優先順序重排

先把兩個層次分開，不然下一個人會來「修正」錯的那一個：

- `strategy.yaml` 的 **objective 是這套策略的及格線**，它這次一個字都沒改，
  也不該因為這件事去改。它問的是「這套規則值不值得繼續跑」。
- **現金流是這個帳戶存在的目的。** 目的不是門檻，但它決定先做哪件事。

改的只有做事的順序。交易參數、objective、門檻，這次全部沒動。

### 為什麼順序要換

用「風險調整後贏 0050」當唯一視角時，最急的事是證明 alpha 是真的。
換成現金流之後，最會殺死這件事的不是 alpha 不夠，是**報酬順序風險**：
一邊提領一邊碰到深的或長的回撤，等於在低點賣掉單位數，本金回不來。
兩條平均報酬一模一樣的路徑，對提領的人來說結果可以差很多。

所以「這套東西能不能穩定吐現金」的證據，排到「它能不能贏指數」前面。

### 新的順序

**1. 一次做完：歷史拉到 60 個月 + 換成機械式標的池**

原本是分開的兩件事，合併是因為它們共用同一個很貴的步驟——抓取限速 4.5 秒，
61 檔 × 36 個月大約兩小時。要重抓就一次把不含事後資訊的標的池也抓進來，
不要抓兩輪。

為什麼排第一：趨勢策略的整個價值主張就是空頭時它在場外，而**這件事從來沒
測過**。目前唯一的壓力事件 2025-04 是 V 型急跌，趨勢過濾在那種形狀下最容易
看起來很行；2022 是慢跌加盤整，那才是會反覆把你掃出場的形狀。對一個要提領
的帳戶來說，「撐不撐得過 2022」是排在 alpha 前面的問題。

同一輪要處理的是標的池的事後選擇偏誤：現在的觀察清單是 2026 年以
「AI 供應鏈」為原則挑的，拿去回測 2024-09 起的歷史，等於已知誰漲了才選誰。
這比「樣本內」更嚴重——它連「當時的你會不會把這檔放進清單」都不成立。
對照組要用機械式規則（0050 成分股，或每日成交量前 N 名），而且成分要取
**當時的**、不是今天的。alpha 在不在，這一組跑完才知道。

順帶：目前有二十幾檔只有 109 根 K，它們在回測裡幾乎沒貢獻樣本。

**2. `backtest.py` 加現金流量測——只加輸出，不動門檻**

現有七條門檻沒有一條在量「這個帳戶能不能穩定吐現金」。勝率、獲利因子、
報酬/回撤講的是策略好不好，不是每個月拿不拿得到錢。至少要印出來的是：

- 月（或季）已實現損益的分布，特別是**連續為負的最長長度**
- **最長「沒有任何部位平倉」的期間**。這套策略的現金只在平倉時回來，
  趨勢過濾讓它空手三個月的話，那三個月就沒有現金流——即使淨值一點都沒掉
- 空手天數的佔比與分布
- 定額提領路徑：每月固定提 X，看淨值曲線與最壞的那條路徑

這是把目標翻譯成看得見的數字，不是換門檻。要不要把其中哪一條變成門檻，
等第 1 項跑完、而且照 objective 的承諾等滿 30 筆再說。

**3. 滾動 12 個月視窗的穩健性檢查**

現在只看全段，一個好事件就能讓整套通過。實測是：前 12 個月（含 2025-04）
曝險調整後大勝，後 12 個月（純多頭）七條裡有兩條不過，而全段輸出印的是
七條全過。對提領來說最壞的那個視窗比平均的視窗重要，所以這條在現金流視角
下比原本更靠前——但仍排在 1、2 之後，因為沒有乾淨的資料，滾動視窗只是把
有偏的數字切得更細而已。

**4. 產業集中：先量測，不要先設上限**

重置前的 5 檔持股是 4 檔金控加 1 檔塑化。10 檔上限若全落在同一個產業，
檔數變多不等於分散，而相關性在崩盤時會一起衝到 1——那正是提領最怕的形狀。
但 `paper.yaml` 的 2026-09-04 changelog 已經寫過為什麼不把它跟別的改動綁
在一起：兩個變數一起動，之後沒辦法歸因。

所以這一輪只在回測輸出加「每個時點的產業分布」與「同產業最大同時持有數」，
先看它到底有多集中。要不要加上限是下一輪的事，而且那是動參數，要記 changelog。
...
這段時間該做的是 1～4 的研究，不是動參數。1～4 全部只碰 `backtest.py` 與歷史
資料，碰不到帳本，所以可以跟這個時鐘並行跑。

技術債（TPEX 除權息要手動補、`fills.py` 沒接進主流程、`mode: auto` 沒人讀）
順序不變，仍排在投資決策項之後。`fills.py` 只有在真的要下真錢那天才會擋路。

### 這個目標最容易帶來的錯誤決定

「追求現金流」很容易被翻譯成「那就改買高股息」。那不是調整，是換掉正在測
的東西：現在這套的現金來自**已實現價差**，不是配息，而且它平均抱 15 根 K，
多數時候根本抱不過除息日。要做股息現金流是另一個 sleeve、另一套規則、另一份
changelog——不要用同一個帳戶和同一份設定混著跑，否則之後沒有一個數字歸得了因。

另外一件**要自己去查清楚**、而且會直接改變稅後到手金額的事：台股的證券交易
所得與股利所得課稅方式不一樣（資本利得目前停徵，股利要併入所得，還可能有二代
健保補充保費）。它不影響策略對錯，但會影響「同樣一塊錢的現金流實際剩多少」。
在比較「靠價差提領」與「靠配息領錢」這兩條路線之前，先確認現行規定，
不要照這段話的記憶去算。
