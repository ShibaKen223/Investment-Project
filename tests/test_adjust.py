...
# 分割還是配息 —— 標籤優先於因子大小
# --------------------------------------------------------------------------
# 因子那條規則隱含「配息不可能讓價格掉超過 40%」。2603 長榮 2023-06-30
# 配 70 元現金（前收 155 → 參考價 85，因子 0.548）打破它，
# 被當成分割的話那天之前的成交量會被放大 1.82 倍，
# 而策略有一條「近 20 日均量 ≥ 500 張」在讀那個數字。

check("純配息：標籤是息，因子再低也不動量",
      adjust._kind_from_label("息", 0.548387) == "dividend")
check("除權：標籤有權，回到因子判斷（大跌＝分割）",
      adjust._kind_from_label("權", 0.335255) == "split")
check("除權：小幅配股仍算配息（因子接近 1）",
      adjust._kind_from_label("權", 0.99) == "dividend")
check("權息一起發：有權就看因子",
      adjust._kind_from_label("權息", 0.55) == "split")
check("沒有標籤時退回因子判斷",
      adjust._kind_from_label("", 0.25) == "split"
      and adjust._kind_from_label("", 0.97) == "dividend")

_evergreen = adjust._actions_from_result_table(
    ["資料日期", "股票代號", "股票名稱",
     "除權息前收盤價", "除權息參考價", "權/息"],
    [["112年06月30日", "2603", "長榮", "155.00", "85.00", "息"]],
    source="twse", table_name="TWSE 除權息結果表",
)
check("2603 那筆解析出來是配息、不動量",
      _evergreen[0].kind == "dividend" and not _evergreen[0].adjusts_volume)
